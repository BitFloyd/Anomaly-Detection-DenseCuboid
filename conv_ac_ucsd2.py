...
if(gs):
    ...
    train_folder = os.path.join(data_store_suffix,'chapter_store_conv','ucsd2_fdata_store_greyscale_bkgsub'+str(tstrides))
    test_data_store = os.path.join(data_store_suffix, 'chapter_store_conv_test','ucsd2_fdata_store_greyscale_test_bkgsub' + str(tstrides))
    nc=1

else:

    train_folder = os.path.join(data_store_suffix,'chapter_store_conv', 'ucsd2_fdata_store_bksgub' + str(tstrides))
    test_data_store = os.path.join(data_store_suffix, 'chapter_store_conv_test', 'ucsd2_fdata_store_test_bkgsub' + str(tstrides))
    nc=3
...
if(notest):
    ...
    if (h_units > 0):
        ae_model = models.Conv_autoencoder_nostream_UCSD_h(model_store=model_store, size_y=size, size_x=size,
                                                           n_channels=1, h_units=h_units, n_timesteps=8, loss=loss, batch_size=batch_size,
                                                           n_clusters=nclusters, lr_model=1e-3, lamda=lamda, gs=gs, notrain=notrain,
        ...
        ae_model = models.Conv_autoencoder_nostream_UCSD_noh(model_store=model_store, size_y=size, size_x=size,
                                                             n_channels=1, n_timesteps=8, loss=loss, batch_size=batch_size,
                                                             n_clusters=nclusters, lr_model=1e-3, lamda=lamda, gs=gs, notrain=notrain,
