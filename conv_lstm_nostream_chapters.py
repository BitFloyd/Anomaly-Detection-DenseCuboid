...
if(nic==0):
    nic=n_chapters
    suffix = 'tstrd_'+str(tstrides)+'_recon_only_h_units'+str(h_units)
...
# Get MODEL
model_store = 'models/convlstm2d_autoencoder_chapters_'+suffix

ae_model = models.Conv_LSTM_autoencoder_nostream(model_store=model_store,loss='bce',h_units=h_units,n_timesteps=5,n_channels=nc,
                                        batch_size=batch_size,n_clusters=10, clustering_lr=1, lr_model=1e-6,lamda=lamda,
                                        lamda_assign=lamda_assign,n_gpus=n_gpus,gs=gs,notrain=False,data_folder=folder,reverse=True)
